...
def test_v2_delete_is_bff_authenticated_owner_and_attempt_scoped():
    source = Path("modal_service/app.py").read_text(encoding="utf-8")
    route = source.split('@service.delete("/v2/mascot/jobs/{job_id}")', 1)[1].split(
        '@service.post("/v2/mascot/jobs/{job_id}/master-generations"', 1
    )[0]

    assert "bff_operation_context" in route
    assert "_ensure_owner(job, identity.user_id)" in route
    assert "job.attempt_id != identity.attempt_id" in route
    assert "JobOperation.DELETE" in route
