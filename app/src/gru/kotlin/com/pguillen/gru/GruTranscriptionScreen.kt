...
@Composable
internal fun GruTranscriptionScreen(prefs: GruPreferences, modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        EngineChoice(
            ...
            selected = requested == TranscriptionEngine.ONLINE_GROQ,
        ...
        EngineChoice(
            ...
            selected = requested == TranscriptionEngine.PRIVATE_LOCAL,
            action = R.string.gru__use_private,
        ) {
        ...
        if (requested == TranscriptionEngine.PRIVATE_LOCAL || current == TranscriptionEngine.PRIVATE_LOCAL) {
            LocalModelSettings(
                ...
                onActivate = { prefs.setEngine(TranscriptionEngine.PRIVATE_LOCAL) },
                onRemove = {
...
private fun EngineChoice(
    ...
    Column(Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            if (selected) Icon(Icons.Default.CheckCircle, stringResource(R.string.gru__selected))
        }
        Text(stringResource(details), style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
        if (!selected) OutlinedButton(onClick = onClick, modifier = Modifier.fillMaxWidth()) { Text(stringResource(action)) }
    }
}
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        when (state) {
            ...
            is WhisperModelState.Installed -> {
                ...
                FilledTonalButton(onClick = onActivate, modifier = Modifier.fillMaxWidth()) { Text(stringResource(R.string.gru__activate_private)) }
...
@Composable
private fun GroqKeyDialog(initialValue: String, onDismiss: () -> Unit, onSave: (String) -> Unit) {
    var value by remember(initialValue) { mutableStateOf(initialValue) }
    var visible by remember { mutableStateOf(false) }
    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text(stringResource(R.string.gru__api_key_label)) },
        text = {
            OutlinedTextField(
                value = value,
        ...
        confirmButton = { TextButton(onClick = { onSave(value.trim()) }, enabled = value.isNotBlank()) { Text(stringResource(R.string.gru__save_key)) } },
